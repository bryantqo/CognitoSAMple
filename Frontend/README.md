# Cognito SAMple Frontend
This is a relaitivly simple project demonstraiting integrating with Cognito (Or potentially any oAuth2/OpenID server). This project was setup with react just to get something running. The main point of interest is the Authentication.js file which contains all of the logic. 

Note that the constructor for the Authentication class takes a configuration object structured as the following:
```
{
    clientId: "12345",
    "scopes": [
...
```
- clientID is obtained for an app client in Cognito. Note that it is case sensitive and it must match the one used serverside to be able to complete the authentication process. For future use it may be helpful to pull this from the api although be careful to not leak the client secret.
- host is the domain for the Cognito user pool
- scopes is a list of scopes to be requested from cognito (Right now it is not implimented)

The frontend will attempt to communicate with the api which is proxied through the /api path. In a production environment this works for small-medium apps but considerations may need to be made for larger applications where a different location for the api is needed. 
